...
def torneo(request):
    ...
    if request.method == 'POST':
        ...
        if formularioTorneo.is_valid():
            ...
            persona = Torneo(nombre=informacion['nombre'], sitio=informacion['sitio'], year=informacion['year'], partidaN=informacion['partidaN'], humano=informacion['humano'], Argentina=informacion['Argentina'], partidaR=informacion['partidaR'], ganador=informacion['ganador'])
            persona.save()
